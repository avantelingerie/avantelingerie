...
export default function OrderConfirmation() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FFFBF8] pt-32 pb-24 px-4 flex flex-col items-center">
        <Skeleton className="w-16 h-16 rounded-full mb-6 bg-[#c59b5f]/10" />
        <Skeleton className="w-3/4 max-w-md h-10 mb-4" />
        <Skeleton className="w-1/2 max-w-sm h-6 mb-12" />
        <Skeleton className="w-full max-w-4xl h-[400px] rounded-2xl" />
      </div>
    );
  }
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-[#FFFBF8] pt-32 pb-24 px-4 flex flex-col items-center justify-center text-center">
        <h1 className="text-3xl font-serif font-bold text-gray-900 mb-4">Pedido não encontrado</h1>
        <p className="text-gray-500 mb-8 max-w-md font-light leading-relaxed">Não conseguimos localizar as informações deste pedido no momento.</p>
        <Button
          onClick={() => navigate('/')}
          className="bg-[#121212] hover:bg-[#c59b5f] text-white hover:text-black border border-[#c59b5f]/30 rounded-2xl px-10 py-6 text-xs font-bold tracking-widest uppercase transition-all duration-300 shadow-md active:scale-95"
        >
          Voltar para a Loja
        </Button>
      </div>
  ...
  return (
    <main className="min-h-screen bg-[#FFFBF8] pb-24 font-sans text-gray-800">
      <Helmet>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 -mt-10 relative z-20 flex flex-col gap-8">
        ...
        <Card className="border border-[#c59b5f]/15 shadow-premium-sm rounded-3xl overflow-hidden bg-white transition-all hover:shadow-[0_12px_30px_rgba(197,155,95,0.04)]">
                ...
                return (
                  <div key={step.id} className="flex md:flex-col items-center gap-4 md:gap-3 relative z-10 w-full md:w-1/4">
                    {index !== statusSteps.length - 1 && (
                      <div className="md:hidden absolute left-6 top-12 bottom-[-2rem] w-0.5 bg-gray-100 z-0">
                    ...
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 transition-all duration-500 ${step.active
                        ? 'bg-gradient-to-br from-[#121212] to-[#1e1e1e] border border-[#c59b5f]/30 text-[#c59b5f] shadow-[0_0_15px_rgba(197,155,95,0.3)] ring-4 ring-[#c59b5f]/10'
                        : 'bg-[#FFFBF8] text-gray-400 border border-gray-200'
                      }`}>
                      <Icon className="w-5 h-5" strokeWidth={1.5} />
                    </div>

                    <div className="flex flex-col md:items-center md:text-center">
                      <span className={`font-serif text-sm md:text-base ${step.active ? 'text-gray-950 font-bold' : 'text-gray-400 font-light'}`}>
                        {step.label}
                      </span>
                      {step.date && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            <Card className="border border-[#c59b5f]/15 shadow-premium-sm rounded-3xl overflow-hidden bg-white h-full transition-all hover:shadow-[0_12px_30px_rgba(197,155,95,0.04)]">
              <div className="bg-[#FFFBF8] px-6 py-5 border-b border-[#c59b5f]/15 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <h2 className="font-serif font-bold text-lg text-gray-950 flex items-center gap-2">
                  <Package className="w-5 h-5 text-[#c59b5f]" strokeWidth={1.5} />
                  Itens do Pedido
                </h2>
                <span className="text-xs font-bold text-[#c59b5f] bg-white px-4 py-1.5 rounded-full border border-[#c59b5f]/20 tracking-wider shadow-sm uppercase">
                  {order.numero_pedido}
                </span>
              </div>
              ...
              <CardContent className="p-0">
                <div className="flex flex-col divide-y divide-gray-100">
                  {order.items.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="p-6 flex items-start gap-4 hover:bg-[#FFFBF8] transition-colors">
                        <div className="w-20 h-24 rounded-2xl bg-muted border border-[#c59b5f]/15 overflow-hidden shrink-0">
                          {image ? (
                        ...
                        <div className="flex flex-col flex-1 justify-between h-full min-h-[6rem]">
                          ...
                          <div className="flex items-end justify-between mt-2">
                            <span className="text-xs font-medium text-gray-400">Qtd: {quantity}</span>
                            <span className="text-base font-bold text-gray-900">{formatPrice(price * quantity)}</span>
                          </div>
                        </div>
                      </div>
                    );
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <Card className="border border-[#c59b5f]/15 shadow-premium-sm rounded-3xl bg-white transition-all hover:shadow-[0_12px_30px_rgba(197,155,95,0.04)]">
              <div className="px-6 py-5 border-b border-gray-100">
                <h2 className="font-serif font-bold text-lg text-gray-900">Resumo Financeiro</h2>
              </div>
              <CardContent className="p-6 flex flex-col gap-4">
                ...
                {order.discount > 0 && (
                  <div className="flex justify-between text-sm text-emerald-800 font-bold bg-emerald-50/50 p-3 rounded-2xl border border-emerald-100">
                ...
                <div className="flex justify-between items-center py-2">
                  <span className="font-bold text-gray-950 font-serif">Total Pago</span>
                  <span className="text-2xl md:text-3xl font-extrabold text-[#c59b5f]">{formatPrice(order.total)}</span>
                </div>

                <div className="mt-2 flex items-center gap-2.5 text-xs text-gray-400 bg-[#FFFBF8] p-4 rounded-2xl border border-dashed border-[#c59b5f]/25 font-light">
                  <CreditCard className="w-4.5 h-4.5 text-[#c59b5f] shrink-0" strokeWidth={1.5} />
                  <span>
                    Pago via <strong className="text-gray-800 uppercase font-extrabold">{(order.paymentMethod === 'credit_card' || order.paymentMethod === 'credito') ? 'Cartão de Crédito' : (order.paymentMethod === 'boleto' ? 'Boleto Bancário' : 'PIX')}</strong>
                    {order.installments > 1 ? ` em ${order.installments}x sem juros` : ''}
                  </span>
                </div>

                {order.paymentMethod === 'boleto' && (
                  <div className="mt-3 bg-amber-50/60 border border-amber-200/50 rounded-2xl p-4 flex gap-3 text-xs text-amber-800 leading-relaxed font-light">
            ...
            <Card className="border border-[#c59b5f]/15 shadow-premium-sm rounded-3xl bg-white transition-all hover:shadow-[0_12px_30px_rgba(197,155,95,0.04)]">
              ...
              <CardContent className="p-6">
                <address className="not-italic text-sm text-gray-500 leading-relaxed font-light">
        ...
        {/* CADEAU / COUPON OFFER */}
        <div className="bg-gradient-to-br from-[#FFFBF8] to-white border border-[#c59b5f]/25 rounded-3xl p-8 md:p-12 text-center shadow-premium relative overflow-hidden transition-all duration-300">
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#c59b5f]/5 rounded-full blur-3xl pointer-events-none"></div>

          <h3 className="text-xl md:text-2xl font-serif font-bold text-gray-950 mb-3 relative z-10">
            Um presente para sua próxima experiência
          </h3>
          <p className="text-gray-500 font-light text-sm md:text-base max-w-xl mx-auto mb-8 relative z-10 leading-relaxed">
          ...
          <div className="inline-flex items-center bg-white border border-[#c59b5f]/30 rounded-2xl p-2.5 shadow-premium-sm relative z-10">
            <span className="px-6 py-2 text-xl font-extrabold tracking-widest text-[#c59b5f] select-all">
              AVANTE10
            </span>
            <Button
              onClick={copyCoupon}
              className="bg-[#121212] hover:bg-[#c59b5f] text-white hover:text-black rounded-xl px-6 py-3.5 text-xs font-bold tracking-widest uppercase transition-all duration-300 shadow-md"
            >
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <Card className="bg-white border border-[#c59b5f]/15 shadow-premium-sm rounded-3xl transition-all duration-300 hover:shadow-[0_12px_30px_rgba(197,155,95,0.04)]">
            <CardContent className="p-8">
              <h3 className="font-serif font-bold text-lg text-gray-950 mb-6 flex items-center gap-2.5">
              ...
              <div className="flex flex-col gap-4">
                <Button
                  ...
                  className="w-full bg-[#25D366]/5 hover:bg-[#25D366]/10 text-[#128C7E] border border-[#25D366]/20 rounded-2xl py-6.5 text-xs font-bold tracking-widest uppercase transition-all duration-300 shadow-sm flex items-center justify-center"
                >
                  <MessageCircle className="w-5 h-5 mr-3" />
                  Falar no WhatsApp
                </Button>

                <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#FFFBF8] border border-[#c59b5f]/10">
                  <Mail className="w-5 h-5 text-gray-400" />
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-gray-900">E-mail de Suporte</span>
                    <span className="text-xs text-gray-400 font-light">contato@avantelingerie.com.br</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#FFFBF8] border border-[#c59b5f]/10">
                  ...
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-gray-900">Horário de Atendimento</span>
                    <span className="text-xs text-gray-400 font-light">Seg a Sex: 09h às 18h</span>
                  </div>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  <div className="w-10 h-10 rounded-full bg-[#c59b5f]/10 border border-[#c59b5f]/20 flex items-center justify-center text-[#c59b5f] shadow-sm">
                    <ShieldCheck className="w-5 h-5" strokeWidth={1.5} />
                  </div>
                  <span className="font-bold text-sm text-gray-900">Compra Secura</span>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-bold text-sm text-gray-900">Troca Facilitada</span>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-bold text-sm text-gray-900">Envio Garantido</span>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-bold text-sm text-gray-900">Suporte Humanizado</span>
        ...
        <div className="mt-20 mb-10 text-center flex flex-col items-center">
          <div className="w-16 h-1 bg-[#c59b5f]/20 rounded-full mb-8"></div>
          ...
          <Button
            onClick={() => navigate('/')}
            className="bg-[#121212] hover:bg-[#c59b5f] text-white hover:text-black border border-[#c59b5f]/30 rounded-2xl px-12 py-7 text-xs font-bold tracking-widest uppercase transition-all duration-300 shadow-xl active:scale-[0.98] group flex items-center justify-center"
          >
            CONTINUAR COMPRANDO
            <ChevronRight className="w-4.5 h-4.5 ml-2 text-[#c59b5f] group-hover:text-black group-hover:translate-x-1 transition-all duration-300" />
          </Button>
        </div>
