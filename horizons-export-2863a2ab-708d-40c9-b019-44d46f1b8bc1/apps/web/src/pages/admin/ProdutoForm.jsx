...
export default function ProdutoForm() {
  ...
  const compressImage = (file) => {
    return new Promise((resolve) => {
      // Se não for imagem (ex: svg), devolve o original
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let savedProduct;
      if (isEdit) {
        savedProduct = await pb.collection('products').update(id, pbFormData, { $autoCancel: false });
      } else {
        savedProduct = await pb.collection('products').create(pbFormData, { $autoCancel: false });
      }
      ...
      toast.info('Sincronizando produto com o Bling...');
      try {
          ...
          <section className="bg-card p-6 rounded-xl border border-[#c59b5f]/10 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-[#c59b5f]/20 pb-2">
              ...
              <label className="text-xs text-[#c59b5f] hover:underline cursor-pointer flex items-center font-bold">
                <Plus className="w-3.5 h-3.5 mr-1" /> Adicionar Fotos
