...
code = code.replace(target, `                      <CategoryMedia video_capa={cat.video_capa} image={cat.image} name={cat.name} />`);
    ...
    <div className="relative w-full h-full bg-white">
      {videos.map((vid, idx) => (
        <video
          key={idx}
          src={vid}
          className={`absolute inset-0 w-full h-full object-contain transition-opacity duration-1000 ease-in-out group-hover/card:scale-105 brightness-[1.05] contrast-[1.05] md:brightness-110 md:contrast-110 ${typeof idx === 'number' && idx === currentIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
          autoPlay
          loop
          muted
          playsInline
...
console.log('HomePage updated - Success');
