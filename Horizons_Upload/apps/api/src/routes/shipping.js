...
router.post('/calculate', async (req, res, next) => {
  try {
    ...
    const meResponse = await axios.post('https://api.melhorenvio.com.br/api/v2/me/shipment/calculate', {
    ...
    if (freeShippingApplied && shippingOptions.length > 0) {
      // Sort by price to find the cheapest option
      const sorted = [...shippingOptions].sort((a, b) => a.price - b.price);
      // Cheaper option (usually Correios PAC or Jadlog Package) gets zerod out
      const cheapest = sorted[0];
      const targetOption = shippingOptions.find(opt => opt.id === cheapest.id);
      if (targetOption) {
        targetOption.price = 0;
        targetOption.isFree = true;
      }
    }
...
router.post('/track', async (req, res, next) => {
  try {
    ...
    if (orderRecord) {
      ...
      } else if (orderRecord.status === 'processando' || orderRecord.status === 'pago') {
