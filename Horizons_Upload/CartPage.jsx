...
export default function CartPage() {
  ...
  const retailSubtotal = cart.reduce((sum, item) => {
    const retailPrice = Number(item.preco_varejo || item.price || 0);
    return sum + (retailPrice * (item.quantity || 0));
  }, 0);

  const wholesaleSubtotal = cart.reduce((sum, item) => {
    const wholesalePrice = Number(item.preco_atacado || item.price_wholesale || ((item.preco_varejo || item.price || 0) * 0.6));
    ...
    try {
      if (isAuthenticated) {
        ...
        if (isLoginMode) {
          ...
          const authData = await signup(upgradeFormData.email, upgradeFormData.password, 'revendedor');
  ...
  const handleIncompletoSubmit = async (e) => {
    ...
    try {
      ...
      updateUser(updated);
      toast.success('👑 Documento salvo e cadastro completo! Mídias liberadas.');
