...
export default function ResellerPortalPage() {
  ...
  const handleActivationSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Perfil Comercial B2B Ativado com sucesso! Bem-vinda à rede oficial!');
    } catch (error) {
      console.error('Erro na ativação de revendedora:', error);
      ...
      toast.success('Seu cadastro comercial B2B provisório foi ativado localmente!');
    } finally {
              ...
              {!currentUser?.cadastro_completo && !isAdmin ? (
                <div className="max-w-3xl mx-auto bg-[#1a1a1a] border border-[#b0874e]/30 rounded-3xl p-8 shadow-2xl space-y-6 text-white text-left">
                  ...
                  <p className="text-xs text-white/70 font-light leading-relaxed">
                    Você já possui acesso ao portal e às ferramentas, porém o download de criativos, fotos de estúdio e Reels das modelos é exclusivo para revendedoras com cadastro comercial completo. Preencha seus dados abaixo para liberar o acesso instantaneamente.
                  </p>
                  ...
                  <form onSubmit={handleActivationSubmit} className="space-y-4">
                    ...
                    <div className="pt-4">
                      ...
                      >
                        {isActivating ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : 'Ativar Downloads de Mídias'}
                      </button>
                    </div>
                  </form>
                </div>
