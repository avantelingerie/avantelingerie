...
const webhookEstoque = async (req, res) => {
  ...
  try {
    ...
    for (const item of estoques) {
      ...
      const saldoAtual = estoqueInfo.saldoFisico !== undefined ? parseInt(estoqueInfo.saldoFisico) : (estoqueInfo.saldo !== undefined ? parseInt(estoqueInfo.saldo) : (estoqueInfo.estoqueAtual !== undefined ? parseInt(estoqueInfo.estoqueAtual) : null));
